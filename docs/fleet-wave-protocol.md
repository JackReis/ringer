...
python3 tools/fleet_wave.py accept manifest-v1.json --prepared-receipt prepared.json \
 ...
 --paperclip-bin PAPERCLIP --ringside-bin RINGSIDE --beads-host HOST \
 --beads-store STORE --beads-claimant CLAIMANT --judge-receipt judge.json \
 --judge-nonce ONE_SHOT_CALLER_CHALLENGE --receipt terminal.json
```
...
The exact state order is `INTAKE → LEDGERED → CLAIMED → MANIFEST-vN → LINTED → DRY-RUN → PAPERCLIP PREPARED RECEIPT → RINGER RUN → INDEPENDENT CHECK REPLAY → FRESH JUDGE` (only for judgmental work) `→ BEADS ACCEPTED/BLOCKED → Paperclip mirror → Ringside GETs`. A prepared receipt may carry the pre-execution states as one immutable composite bundle, but every transition is separately predecessor-chained; it never claims a walkthrough. Judgmental criteria require a fresh never-resumed Judge, a configured harness-secret HMAC over the complete attestation, normalized principal/session non-overlap with controller and replay checker, and a constant-time match of the one-shot `--judge-nonce`. The signed exact attestation binds attempt, manifest, execute receipt, run state, nonce, timezone-aware session/evidence/issuance/judgment times, and an expiry no more than ten minutes after issuance; acceptance permits at most five-minute-old judgments and 30 seconds of clock skew. There is no receipt-only pseudo-terminal: acceptance always closes and reads back Beads. Projection degradations use unique stage-specific immutable receipts.
